...
def _validate_entry(
    ...
    enum_name = "HOPDiag_" + enum_suffix
...
def render_data_c(entries: list[dict[str, object]], rel_source: str) -> str:
    ...
    lines.append("")
    lines.append("HOP_API_BEGIN")
    lines.append("")
    lines.append("typedef struct {")
    lines.append("    const char* id;")
    lines.append("    const char* messageFmt;")
    lines.append("    const char* hint;")
    lines.append("    HOPDiagType  type;")
    lines.append("    uint8_t     argCount;")
    lines.append("} HOPDiagInfo;")
    lines.append("")
    lines.append("static const HOPDiagInfo g_hopDiagInfo[HOPDiag__COUNT] = {")
    lines.append('    [HOPDiag_NONE] = { "", "no error", NULL, HOPDiagType_ERROR, 0 },')
    for e in entries:
        hint = "NULL" if e["hint"] == "" else f"\"{c_escape(str(e['hint']))}\""
        diag_type = "HOPDiagType_WARNING" if e["type"] == "warning" else "HOPDiagType_ERROR"
        lines.append(
    ...
    lines.append("};")
    lines.append("")
    lines.append("const char* HOPDiagId(HOPDiagCode code) {")
    lines.append("    uint32_t i = (uint32_t)code;")
    lines.append("    if (i >= (uint32_t)HOPDiag__COUNT || g_hopDiagInfo[i].id == NULL) {")
    lines.append('        return "HOP????";')
    lines.append("    }")
    lines.append("    return g_hopDiagInfo[i].id;")
    lines.append("}")
    lines.append("")
    lines.append("const char* HOPDiagMessage(HOPDiagCode code) {")
    lines.append("    uint32_t i = (uint32_t)code;")
    lines.append("    if (i >= (uint32_t)HOPDiag__COUNT || g_hopDiagInfo[i].messageFmt == NULL) {")
    lines.append('        return "unknown diagnostic";')
    lines.append("    }")
    lines.append("    return g_hopDiagInfo[i].messageFmt;")
    lines.append("}")
    lines.append("")
    lines.append("const char* _Nullable HOPDiagHint(HOPDiagCode code) {")
    lines.append("    uint32_t i = (uint32_t)code;")
    lines.append("    if (i >= (uint32_t)HOPDiag__COUNT) {")
    lines.append("        return NULL;")
    lines.append("    }")
    lines.append("    return g_hopDiagInfo[i].hint;")
    lines.append("}")
    lines.append("")
    lines.append("HOPDiagType HOPDiagTypeOfCode(HOPDiagCode code) {")
    lines.append("    uint32_t i = (uint32_t)code;")
    lines.append("    if (i >= (uint32_t)HOPDiag__COUNT) {")
    lines.append("        return HOPDiagType_ERROR;")
    lines.append("    }")
    lines.append("    return g_hopDiagInfo[i].type;")
    lines.append("}")
    lines.append("")
    lines.append("uint8_t HOPDiagArgCount(HOPDiagCode code) {")
    lines.append("    uint32_t i = (uint32_t)code;")
    lines.append("    if (i >= (uint32_t)HOPDiag__COUNT) {")
