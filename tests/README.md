...
- `pretest_fmt_exempt_input` (bool, default `false`): exclude this fixture input from the pre-test formatter gate
- `pretest_fmt_exempt_paths` (array of strings): for directory inputs, exclude specific relative `.hop` paths from the pre-test formatter gate

## Pre-test formatting gate

`tools/test.py run` executes a formatting gate before running tests:

- runs `hop fmt --check` against selected test source files under `examples/`, `lib/`, and `tests/`
...
The formatter gate builds and uses the release `hop` binary for speed. Test executions still use the binary from `--build-dir`.
Use `--preflight-jobs` to control formatter parallelism. It defaults to the CPU count and is independent from `--jobs`, which controls test execution parallelism.
