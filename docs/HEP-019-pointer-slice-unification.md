# HEP-19 pointer and slice unification (completed)
...
HEP-19 simplifies reference-like types into two mutability forms:
...
This proposal supersedes the HEP-2 split between `mut&T`/`mut&[T N]` and `mut[T]`.
...
HEP-19 keeps one mutability axis:
...
- Changing nullability defaults in this HEP.
...
- pointee of `*` / `&` (`*[T]`, `&[T]`)
- other places explicitly marked unsized-capable by future HOPPs
...
- rejected for variable-size aggregate types (from HEP-1)
...
## Interaction with HEP-1 variable-size structs

HEP-1 dependent trailing fields stay unchanged:
...
2. Typechecker:
   ...
   - coercions listed in this HEP are implemented.
3. Codegen/runtime:
...
1. Nullability policy for pointers and refs:
   - unchanged in HEP-19; non-null-by-default is deferred to a dedicated future HEP.
