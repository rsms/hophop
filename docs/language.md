...
- [EXPR-FN-001][Provisional] `fn(...) [Result] Block` in expression context forms an anonymous function value. Function values use a uniform `(code, data)` representation; non-capturing functions use null data, and capturing anonymous functions use implementation-managed stack closure data.
...
- [EXPR-FN-004][Provisional] Anonymous functions may reference package-scope variables, visible constants, and enclosing runtime bindings that can be captured by a non-escaping stack closure. Capturing function values MUST NOT escape the defining runtime scope.
...
- [STMT-FN-002][Provisional] Local named functions follow the same stack-closure capture and escape rules as anonymous functions.
