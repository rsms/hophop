# HEP-4 type functions (completed)

## Summary

HEP-4 adds type-function call syntax as sugar:
...
HopHop already uses package-level functions as its core abstraction. This improves call-site ergonomics without introducing methods as a new declaration kind.
...
HEP-4 resolves overloads from visible functions sharing the target call name.
...
`hop` should print a short candidate list for overload errors (bounded length).

---

## Interaction with HEP-6 struct composition
