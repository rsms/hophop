# SLP-34 foreign linkage directives

## Summary

SLP-34 adds a small directive syntax for foreign imports and exports:
...
SLP-34 defines three concrete directives:
...
This replaces the earlier SLP-34 draft based on special `import "external/..."` syntax and
`pub "c"`.

## Motivation

SL needs a direct way to express:

- foreign functions and globals provided by C-ABI link inputs
- Wasm imports provided at module instantiation time
- SL functions exported for foreign callers
...
- Export selected SL functions for foreign callers.
...
- arbitrary per-declaration ABI strings beyond the concrete directives in this SLP
...
- SLP-34 only defines semantics for directives attached to top-level `fn`, `const`, and `var`
  declarations.
- The parser accepts a general directive surface, but the checker only recognizes the concrete
  directives defined in this SLP.
- In SLP-34, all directive arguments used by the standardized foreign-linkage directives are string
  literals.
...
SLP-7 path imports and named-symbol imports continue to work unchanged.
...
- the local SL binding name is the declaration name, not the `field` string
...
- the local SL binding name is the declaration name, not the `symbol` string
...
Here the local SL name is `crc32`, while the foreign symbol name is `zlib_crc32`.

### 5. `@export(name)`

`@export(name)` declares that the following SL function is exported for foreign callers.
...
- exactly one argument is required
- the argument must be a string literal
- `@export` does not change the SL declaration name
- `@export` and `pub` are orthogonal in intent, but SLP-34 requires `pub fn` for the initial
  design
...
This avoids introducing a hidden-parameter foreign ABI rule in SLP-34.
...
Mutability follows the SL declaration form:
...
- `slc genpkg:c` is allowed to emit C for `@wasm_import` even when no downstream target triple is
  known yet
- when `slc` uses the C backend and the final target is known not to be Wasm, `@wasm_import` is a
  build-time error
...
- `<local_c_name>` follows the local SL binding name or the backend's usual internal-mangling rule
- the foreign Wasm names come from directive arguments, not from the SL declaration name
...
Migration from the earlier SLP-34 draft is mechanical:
...
1. Add directive syntax `@directive` and `@directive(...)`, attached to the following top-level
   declaration.
2. SLP-34 standardizes only `@wasm_import`, `@c_import`, and `@export`.
