# HEP-11 function type syntax (completed)

## Summary

HEP-11 adds function types to the type grammar:
...
HopHop currently cannot express function-typed fields, variables, or parameters. This forces ABI hacks
...
- Parameter runs are supported for consistency with HEP-8:
...
This keeps behavior simple and avoids introducing variance rules in this HEP.
...
```c
__hop_uint (*impl)(Allocator* self, ...);
```
...
- Extend type parsing to accept `HOPTok_FN` in `Type`.
- Add `HOPAst_TYPE_FN` node kind.
- Parse parameter groups in type context (same group shape as HEP-8), plus optional return type.
...
- Support:
  ...
  - assignment checks (exact signature match in HEP-11)
