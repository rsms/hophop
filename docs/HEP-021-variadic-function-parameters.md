# HEP-21 variadic function parameters

## Summary

HEP-21 adds Go-style variadic function parameters.
...
This proposal is intended as a backend-agnostic language feature and a prerequisite for pure-HopHop
formatting and logging APIs.

## Motivation

HopHop currently requires fixed arity at each function declaration. That makes APIs like formatting,
...
- Implicit dynamic boxing as part of this HEP.
...
This HEP treats `ys` as a normal slice value for operations (`len`, indexing, iteration rules), even
if HopHop internally uses a dedicated representation.
...
2. Should variadic slice be mutable or read-only by default in HopHop?
