# HEP-7 import declarations (completed)

## Summary

HEP-7 revises `import` to support:
...
HEP-7 improves readability and scales better for packages with mixed usage patterns.
...
- `import "hophop/feature/<name>"` remains a compiler directive import.
- `import "platform"` remains a built-in package import.
- Feature imports must be path-only (`import "hophop/feature/<name>"`) and cannot use `as`
  or `{...}`.
- `platform` follows normal HEP-7 import rules (alias and named imports are allowed).
...
3. Built-in `platform` uses the same alias rules as normal packages in HEP-7.
