# HEP-25 anytype parameters and variadic packs (completed)

## Summary

HEP-25 introduces `anytype` as a function-parameter placeholder for unconstrained argument types.
...
HopHop currently has no way to express APIs that accept heterogeneous argument types while preserving
...
In this HEP, `anytype` is only valid in function parameter positions:
...
### 5. Interaction with `const` parameters (HEP-24)
...
1. `args[i]` on `...anytype` is limited to const-evaluable index expressions in this HEP.
...
2. Array/slice spread into `...anytype` is not planned in this HEP.
