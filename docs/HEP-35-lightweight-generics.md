# HEP-35 lightweight parametric generics

## Summary

HEP-35 adds lightweight parametric generics for the most common cases of reusable types and
functions.
...
HopHop already has several generic-adjacent building blocks:
...
- each type argument must be a well-formed HopHop type
...
- HopHop already has an explicit and simpler spelling for this use case: ordinary functions with
  `type` parameters
...
HEP-35 generics are for inferred parametric reuse. Explicit `type` parameters remain the
...
- use HEP-35 generics for same-type reusable APIs
