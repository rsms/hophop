# HEP-24 const function parameters (completed)

## Summary

HEP-24 adds a `const` modifier for function parameters to mean:
...
Current HopHop has const-eval behavior, but there is no direct way to declare this requirement in a
function signature.
...
under Reference-hop const-eval rules.
