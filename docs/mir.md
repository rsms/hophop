...
- The const-block MIR executor path now also drops `resolveIdent` and `resolveCall` entirely when a lowered MIR program has no remaining generic `LOAD_IDENT` / `CALL` instructions, so fully lowered const blocks no longer keep that dynamic-resolution callback dependency alive.
