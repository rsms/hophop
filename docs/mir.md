...
- The consteval-side MIR env now also supplies sequence-length and iterator hooks for simple sequence `for in`, so const calls and `const { ... }` blocks can keep string/tuple-style iteration on MIR instead of dropping back to `ctfe_exec` immediately.
