# HEP-10 variable type inference (completed)

## Summary

HEP-10 allows omitting explicit types in `var` and `const` declarations when an initializer is
present.
...
This HEP does not add new public-API visibility rules; it only specifies inference behavior when
