...
SLP-12 introduces context-typed capability passing with two syntax additions:
...
SL needs a unified way to pass capabilities and request-scoped values (allocator, filesystem,
...
- No new declaration kind in SLP-12.
- No context-polymorphic generics in SLP-12.
- No block-form context rebinding construct in SLP-12.
...
- For a given function name/signature, all declarations and the definition must use the same
  `context` clause.
- `context` does not create a new overload dimension in SLP-12.

---

## Noise mitigation options

SLP-12 can stay strict while keeping code readable. Options:
...
SLP-12 is designed to work immediately with named context types.
