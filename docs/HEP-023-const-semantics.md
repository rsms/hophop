# HEP-23 const semantics (completed)

## Summary

HEP-23 proposes changing `const name = val` to mean compile-time constant in all scopes
...
Numeric literal default-type changes (`const_int` / `const_float`) are split into HEP-27.

## Current behavior (Reference-hop, git 0020658990470fd1a9e7f957049a0cef4814b511)
...
- code can pass `hop check` and fail later at C compile due to const assignment
...
- Reuse `HOP2055` (`const initializer must be a const expression`) for non-const-evaluable const
...
2. Assignments to const bindings that currently pass `hop check` will fail at typecheck (earlier,
   better diagnostic).
