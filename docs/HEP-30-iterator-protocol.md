# HEP-30 iterator protocol for `for ... in` user types (completed)

## Summary

HEP-30 extends `for ... in` (HEP-29) with a protocol for user-defined iterable types.
...
HEP-29 covers sequence-like types (`len` + `[]`) but leaves user-defined/infinite iteration out of scope.
...
1. If `EXPR` is built-in sequence-iterable, use HEP-29 sequence lowering.
...
This revision is intentionally breaking versus previous HEP-30 drafts:
...
3. Interaction:
   ...
   - sequence types still use HEP-29 path
