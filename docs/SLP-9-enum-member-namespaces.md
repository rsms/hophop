...
SLP-9 changes enum member lookup from package-global names to enum-scoped names.
...
- Existing numeric compatibility/coercion behavior for enums remains unchanged in this SLP.
...
- C enum enumerators are global in C; backend should mangle enumerator names to avoid collisions
  across enums while preserving SL-level `Enum.Member` semantics.
