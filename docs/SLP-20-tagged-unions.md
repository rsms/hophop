...
SLP-20 extends `enum` so each variant may optionally carry a payload, using struct-like field syntax
...
SLP-20 also proposes `switch` case variant patterns with narrowing and alias binding:
...
Current SL enums are tag-only. Many real models need tagged choices with data:
...
- If SLP-16 contextual enum literals are enabled, `.Variant{ ... }` is allowed where expected enum
