# SLP-36 filename build tags

## Summary

SLP-36 adds filename build tags for directory packages. A tagged file is included only when all of
its tags match the current package build. Untagged `.sl` files are always included.
...
```text
impl[wasm32].sl
impl[playbit,!aarch64].sl
helper[testing].sl
```

Comma means `AND`. A leading `!` negates one tag, so `impl[playbit,!aarch64].sl` is included only
when `playbit` is active and `aarch64` is inactive.

## Motivation

SL packages need a small conditional compilation mechanism for platform support, architecture
...
- Keep single-file CLI input explicit: a direct `foo[wasm32].sl` input is loaded even when `wasm32`
  is inactive.
...
Tags are recognized only in the final basename segment immediately before `.sl`.

Valid examples:

```text
foo[wasm32].sl
foo[playbit,!aarch64].sl
foo[testing].sl
```
...
If the expression does not match, the file is skipped. If a directory package has `.sl` files but
none match after filtering, loading fails with `no matching .sl files found in <dir>`.
...
```text
slc checkpkg [--platform <target>] [--arch <name>] <pkgdir|srcfile>
slc mir [--platform <target>] [--arch <name>] <pkgdir|srcfile>
slc genpkg:c [--platform <target>] [--arch <name>] <pkgdir|srcfile>
slc compile [--platform <target>] [--arch <name>] <pkgdir|srcfile>
slc run [--platform <target>] [--arch <name>] <pkgdir|srcfile>
```
