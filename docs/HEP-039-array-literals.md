...
`new`, but a bare array literal does not imply dynamic allocation.
...
- Allow `new [expr, ...]` for initialized heap arrays.
...
ArrayElementSep  = "," | ";" .
NewExpr          = "new" ( "[" Type Expr "]" | ArrayLit | Type [ "{" [ FieldInitList ] "}" ] )
                   [ "in" Expr ] .
```
...
```hop
var q *[int] = new [1, 2, 3] // dynamic-size array with 3 elements
```
...
```hop
var r *[int 64] = new [1, 2, 3]
```

The allocator behavior follows the active `new` rules. If HEP-38 is active, plain `new` uses GC
allocation and `new ... in allocator` uses explicit manual allocation.
...
- `new [expr, ...]` can produce `*[T]` only in an expected `*[T]` context
...
`[` is not currently a valid primary-expression starter, so `[expr, ...]` adds a new expression form
...
`new [` needs parser disambiguation between existing `new [T n]` allocation syntax and new
`new [expr, ...]` literal allocation syntax. The same expression/type-context split already exists
...
- accept `var p = new [1, 2, 3]`
- accept `var q *[int] = new [1, 2, 3]`
