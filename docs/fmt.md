...
- Local single-name `var` declarations with an initializer and no required explicit type format as short assignment syntax (`name := expr`) for simple literals, local identifiers, and explicit compound literals. Top-level declarations, grouped declarations, multi-name declarations, declarations without initializers, call initializers, array/alloc initializers, and `for` header initializers keep `var` syntax.
