# HEP-8 parameter type runs (completed)

## Summary

HEP-8 adds compressed parameter type runs in function parameter lists.
...
`EXPECTED_TYPE` may still be used as parser fallback, but HEP-8 recommends a dedicated diagnostic
...
Current parser reads one parameter as `Ident Type`. HEP-8 should parse grouped names before type:

1. Read one or more names separated by `,`.
2. Require one trailing type for that group.
3. Emit one `HOPAst_PARAM` node per name.

Important:

- Emit an explicit type node for each emitted `HOPAst_PARAM` (no shared child node).
- Keep emitted AST in canonical expanded form so downstream phases do not need HEP-8-specific logic.
