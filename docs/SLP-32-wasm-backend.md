# SLP-32 MIR-to-Wasm backend

## Summary

SLP-32 adds a new `wasm` compiler backend that lowers checked SL packages through
backend-facing MIR (`SLMirProgram`) to a WebAssembly module.
...
Today SL has one production codegen backend: C11.

That works well for portability, but it has limits:

- it requires a host C toolchain after `slc`
...
- Add a `wasm` backend to `slc`.
- Lower from checked package state through `SLMirProgram`, not through C.
...
This SLP defined the backend shape and bring-up plan. The active manifest now covers
that intended bootstrap surface; remaining Wasm work is follow-up outside SLP-32.
...
Wasm subset through an internal `sl_main` wrapper and reserved result buffer for aggregate,
...
surface for SLP-32. Non-top-level or capturing function values remain post-SLP follow-up work.
...
If `SLMirProgramNeedsDynamicResolution(...)` is true after lowering/rewrite, Wasm codegen
...
```sh
slc genpkg:wasm <dir|file.sl> [out.wasm]
```
...
- A debug-only text form such as WAT may be added later, but is not required by this SLP.
...
The backend should be implemented in C inside `libsl`, without requiring libc, libc++, or a host
Wasm toolchain at runtime.

Recommended structure:

- a small handwritten Wasm binary writer
- a Wasm-specific lowering pass from `SLMirProgram`
- a later optional lower CFG/SSA IR for optimization before final Wasm emission
- use the existing `slc mir <package-dir|file.sl>` text dump as the primary development/debugging
  surface for backend-facing MIR

Rationale:

- the Wasm binary format is regular enough that writing modules directly is practical
- a handwritten emitter keeps `libsl` freestanding and small
- the difficult parts are lowering, ABI, structured control flow, data layout, and optimization,
  not byte emission itself

This SLP treats the Wasm encoder as implementation detail. The important requirement is that the
implementation remain compatible with the freestanding `libsl` constraints.
...
```sh
slc mir <package-dir|file.sl>
```

This prints the current backend-facing `SLMirProgram` in a deterministic text format.
...
- when adding or debugging Wasm lowering, first inspect `slc mir` output for the same fixture
- when adding new MIR rewrites for Wasm readiness, prefer adding or updating `slc mir` golden tests
...
- SL pointers and references lower to 32-bit linear-memory addresses
...
The backend targets `SLMirProgram`, not raw AST.

For v1, the Wasm backend may require all of the following:

- validated `SLMirProgram`
...
`slc mir` is the intended way to inspect whether those requirements are actually met for a given
...
This SLP does not require one specific lower IR design, but it explicitly allows:
...
This SLP intentionally separates two concerns:
...
- exporting `memory`
- exporting `sl_main` or another deterministic entry wrapper
...
`import "platform"` remains a language-level pseudo package as defined by SLP-5 and SLP-17.

This SLP does not define a new stable Wasm `platform` package.
...
- a future SLP can define stable Wasm platform packages after backend experience exists
...
This backend should not require a third-party Wasm compiler library as part of `libsl`.
...
- do not require Binaryen, WABT, or another external Wasm library to build or run `libsl`
...
- compare emitted Wasm against `slc mir` output while debugging lowering mismatches

Examples of non-goals for v1:

- linking Binaryen into `libsl`
...
- extend `SLCodegenBackend` output handling so a backend can emit binary bytes, not only C text
- register backend name `wasm`
- add CLI support for `genpkg:wasm`

### Phase 2: MIR package formation for Wasm

- add or extend package-level MIR lowering so codegen gets one validated `SLMirProgram`
- ensure Wasm-targeted lowering can reject unresolved dynamic operations before emission
- keep `slc mir` output useful and stable enough to debug that package-level MIR formation
...
1. Backend formation:
   ...
   - corresponding `slc mir` output clearly shows the unresolved operations that block Wasm codegen
2. Structural output:
   - emitted module contains expected sections, exports, and deterministic data layout
   - `slc mir` goldens exist for representative backend-facing MIR programs used by Wasm tests
...
- direct Wasm execution support in `slc run`
