...
static char* _Nullable BuildDeclTextForNode(
    ...
    if (!isPubSurface) {
        return SLCDupSlice(file->source, DeclTextSourceStart(file, nodeId), n->end);
    }
...
static char* _Nullable MakeUniqueImportAlias(const SLPackage* pkg, const char* preferred) {
    ...
    {
        return SLCDupCStr(preferred);
    }
    if (preferred == NULL || preferred[0] == '\0' || !IsValidIdentifier(preferred)) {
        preferred = "imp";
    }
    alias = SLCDupCStr(preferred);
    if (alias == NULL) {
        return NULL;
    }
...
static char* _Nullable DupPubDeclText(const SLParsedFile* file, int32_t nodeId) {
    ...
    return SLCDupSlice(file->source, start, end);
}
...
static int AddDeclFromNode(
        ...
        && file->ast.nodes[firstChild].kind == SLAst_NAME_LIST)
    {
        ...
        for (i = 0; i < nameCount; i++) {
            ...
            name = SLCDupSlice(file->source, nameAst->dataStart, nameAst->dataEnd);
    ...
    {
        char* name = SLCDupSlice(file->source, n->dataStart, n->dataEnd);
...
static int ProcessParsedFile(SLPackage* pkg, uint32_t fileIndex) {
    ...
    while (child >= 0) {
        ...
        if (n->kind == SLAst_IMPORT) {
            ...
            if (aliasNode != NULL) {
                bindName = SLCDupSlice(file->source, aliasNode->dataStart, aliasNode->dataEnd);
                if (bindName == NULL) {
                    free(importPath);
                    return ErrorSimple("out of memory");
                }
            ...
            while (importChild >= 0) {
                ...
                if (ch->kind == SLAst_IMPORT_SYMBOL) {
                    ...
                    char*   sourceName = SLCDupSlice(file->source, ch->dataStart, ch->dataEnd);
                    char*   localName;
                    if (sourceName == NULL) {
                        return ErrorSimple("out of memory");
                    }
                    if (localAliasNode >= 0 && ast->nodes[localAliasNode].kind == SLAst_IDENT) {
                        ...
                        localName = SLCDupSlice(file->source, ln->dataStart, ln->dataEnd);
                    } else {
                        localName = SLCDupCStr(sourceName);
                    }
                    if (localName == NULL) {
                        free(sourceName);
                        return ErrorSimple("out of memory");
...
static int ValidateAndFinalizeImportSymbols(SLPackage* pkg) {
    ...
    for (i = 0; i < baseLen; i++) {
        ...
        for (j = 0; j < dep->pubDeclLen; j++) {
            ...
            if (matchCount > 0) {
                char* sourceName = SLCDupCStr(sym->sourceName);
                char* localName = SLCDupCStr(sym->localName);
...
static int AddPackageSlot(SLPackageLoader* loader, const char* dirPath, SLPackage** outPkg) {
    ...
    if (EnsureCap((void**)&loader->packages, &loader->packageCap, needCap, sizeof(SLPackage)) != 0)
    {
        return -1;
    }
    pkg = &loader->packages[loader->packageLen++];
    memset(pkg, 0, sizeof(*pkg));
    pkg->dirPath = SLCDupCStr(dirPath);
    if (pkg->dirPath == NULL) {
        return -1;
    }
...
static int ApplyTextRewrites(
    ...
    *outText = NULL;
    ...
    if (rewriteLen == 0) {
        *outText = SLCDupSlice(text, 0, textLen);
...
int LoadPackageForFmt(
    ...
    *outEntryPkg = NULL;
    ...
    loader.platformTarget = SLCDupCStr(
