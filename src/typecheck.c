...
static int SLTCFailAssignToConst(SLTypeCheckCtx* c, int32_t lhsNode) {
...
static int32_t SLTCGetStrPtrType(SLTypeCheckCtx* c, uint32_t start, uint32_t end) {
    if (c->typeStr < 0) {
        return -1;
    }
...
static uint8_t SLTCTypeTagKindOf(SLTypeCheckCtx* c, int32_t typeId) {
    const SLTCType* t;
    int32_t         declNode;
...
static uint64_t SLTCEncodeTypeTag(SLTypeCheckCtx* c, int32_t typeId) {
...
static int SLTCDecodeTypeTag(SLTypeCheckCtx* c, uint64_t typeTag, int32_t* outTypeId) {
    uint64_t id;
    int32_t  typeId;
...
static int32_t SLTCResolveTypeValueName(SLTypeCheckCtx* c, uint32_t start, uint32_t end) {
...
static int32_t SLTCFindCoreNamedTypeBySuffix(SLTypeCheckCtx* c, const char* suffix) {
    uint32_t i;
    for (i = 0; i < c->typeLen; i++) {
        const SLTCType* t = &c->types[i];
        if (t->kind != SLTCType_NAMED) {
...
static int32_t SLTCFindNamedTypeBySuffix(SLTypeCheckCtx* c, const char* suffix) {
    uint32_t i;
    for (i = 0; i < c->typeLen; i++) {
        const SLTCType* t = &c->types[i];
        if (t->kind != SLTCType_NAMED) {
...
static int32_t SLTCFindReflectKindType(SLTypeCheckCtx* c) {
    uint32_t i;
...
static int32_t SLTCFindFmtValueType(SLTypeCheckCtx* c) {
    uint32_t i;
...
static int SLTCTypeIsFmtValue(SLTypeCheckCtx* c, int32_t typeId) {
    int32_t valueType;
    if (c == NULL || typeId < 0) {
        return 0;
    }
...
static int SLTCResolveReflectedTypeValueExpr(
    SLTypeCheckCtx* c, int32_t exprNode, int32_t* outTypeId);
...
static int SLTCTupleTypeMatchesSignature(
...
static int32_t SLTCInternTupleType(
    SLTypeCheckCtx* c,
...
static int SLTCResolveConstCall(
    void*              ctx,
    uint32_t           nameStart,
    uint32_t           nameEnd,
...
static void SLTCConstSetReason(
...
static void SLTCConstSetReasonNode(SLTCConstEvalCtx* evalCtx, int32_t nodeId, const char* reason) {
    SLTypeCheckCtx* c;
    if (evalCtx == NULL) {
        return;
    }
...
static void SLTCAttachConstEvalReason(SLTypeCheckCtx* c) {
...
static int SLTCResolveConstIdent(
    void*        ctx,
    uint32_t     nameStart,
    uint32_t     nameEnd,
    SLCTFEValue* outValue,
...
static int SLTCConstBuiltinSizeBytes(SLBuiltinKind b, uint64_t* outBytes) {
    if (outBytes == NULL) {
        return 0;
    }
    switch (b) {
...
static int SLTCConstBuiltinAlignBytes(SLBuiltinKind b, uint64_t* outAlign) {
    uint64_t size = 0;
...
static uint64_t SLTCConstAlignUpU64(uint64_t v, uint64_t align) {
    if (align == 0) {
        return v;
    }
...
static int SLTCConstTypeLayout(
...
static int SLTCConstEvalSizeOf(
    SLTCConstEvalCtx* evalCtx, int32_t exprNode, SLCTFEValue* outValue, int* outIsConst) {
    SLTypeCheckCtx*  c;
    const SLAstNode* n;
    int32_t          innerNode;
...
static int SLTCConstEvalTypeOf(
    SLTCConstEvalCtx* evalCtx, int32_t exprNode, SLCTFEValue* outValue, int* outIsConst) {
    SLTypeCheckCtx*  c;
...
static int SLTCConstEvalTypeNameValue(
...
static int SLTCConstEvalTypeReflectionCall(
    SLTCConstEvalCtx* evalCtx, int32_t exprNode, SLCTFEValue* outValue, int* outIsConst) {
    SLTypeCheckCtx*  c;
    int32_t          calleeNode;
    const SLAstNode* callee;
...
static int SLTCEvalConstExecExprCb(
    void* ctx, int32_t exprNode, SLCTFEValue* outValue, int* outIsConst) {
    return SLTCEvalConstExprNode((SLTCConstEvalCtx*)ctx, exprNode, outValue, outIsConst);
}

static int SLTCEvalConstExecResolveTypeCb(void* ctx, int32_t typeNode, int32_t* outTypeId) {
    SLTCConstEvalCtx* evalCtx = (SLTCConstEvalCtx*)ctx;
...
static int SLTCEvalConstExecInferValueTypeCb(
    void* ctx, const SLCTFEValue* value, int32_t* outTypeId) {
    SLTCConstEvalCtx* evalCtx = (SLTCConstEvalCtx*)ctx;
    SLTypeCheckCtx*   c;
...
static int32_t SLTCFindConstCallableFunction(
...
static int SLTCConstStringExpr(
    SLTypeCheckCtx* c,
    int32_t         nodeId,
...
static int SLTCFnNodeHasTypeParamName(
...
static int SLTCResolveActiveTypeParamType(
...
static int SLTCTypeSupportsFmtReflectRec(SLTypeCheckCtx* c, int32_t typeId, uint32_t depth) {
    uint32_t i;
    typeId = SLTCResolveAliasBaseType(c, typeId);
...
static int SLTCTypeIsFreeablePointer(SLTypeCheckCtx* c, int32_t typeId) {
    typeId = SLTCResolveAliasBaseType(c, typeId);
    if (typeId < 0 || (uint32_t)typeId >= c->typeLen) {
        return 0;
    }
...
static int32_t SLTCUnwrapCallArgExprNode(SLTypeCheckCtx* c, int32_t argNode) {
    const SLAstNode* n;
...
static int SLTCCollectCallArgInfo(
    SLTypeCheckCtx*  c,
    int32_t          callNode,
    int32_t          calleeNode,
    int              includeReceiver,
...
static void SLTCCallMapErrorClear(SLTCCallMapError* err) {
    err->code = 0;
    err->start = 0;
    err->end = 0;
    err->argStart = 0;
...
static int SLTCMapCallArgsToParams(
    SLTypeCheckCtx*        c,
...
static int SLTCPrepareCallBinding(
    SLTypeCheckCtx*        c,
    const SLTCCallArgInfo* callArgs,
    uint32_t               argCount,
    const uint32_t*        paramNameStarts,
...
static int SLTCResolveDependentPtrReturnForCall(
...
static int SLTCResolveIdentifierExprType(
    SLTypeCheckCtx* c,
    uint32_t        nameStart,
    uint32_t        nameEnd,
    uint32_t        spanStart,
...
static int SLTCExprIsConstAssignTarget(SLTypeCheckCtx* c, int32_t exprNode) {
    const SLAstNode* n;
    if (exprNode < 0 || (uint32_t)exprNode >= c->ast->len) {
        return 0;
    }
        ...
        && c->defaultFieldCurrentIndex < c->defaultFieldCount)
    {
        for (i = 0; i < c->defaultFieldCount; i++) {
            ...
            if (f->kind != SLAst_FIELD
                || !SLNameEqSlice(c->src, f->dataStart, f->dataEnd, n->dataStart, n->dataEnd))
            {
                continue;
            }
...
static int SLTCTypeExpr_STRING(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
...
static int SLTCTypeExpr_COMPOUND_LIT(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
    (void)n;
    return SLTCTypeCompoundLit(c, nodeId, -1, outType);
}

static int SLTCTypeExpr_CALL_WITH_CONTEXT(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
    int32_t savedActive = c->activeCallWithNode;
...
static int SLTCTypeExpr_SIZEOF(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
...
static int SLTCTypeExpr_FIELD_EXPR(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
...
static int SLTCTypeExpr_BINARY(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
...
static int SLTCTypeExpr_UNWRAP(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
    int32_t inner = SLAstFirstChild(c->ast, nodeId);
    int32_t innerType;
    (void)n;
...
static int SLTCTypeExpr_TUPLE_EXPR(
    SLTypeCheckCtx* c, int32_t nodeId, const SLAstNode* n, int32_t* outType) {
    int32_t  child = SLAstFirstChild(c->ast, nodeId);
    uint32_t elemCount = 0;
    (void)n;
...
static int SLTCValidateConstInitializerExprNode(SLTypeCheckCtx* c, int32_t initNode) {
    SLTCConstEvalCtx evalCtx;
    SLCTFEValue      value;
    int              isConst = 0;
    int              rc;
...
static int SLTCValidateLocalConstVarLikeInitializers(
...
static int SLTCValidateTopLevelConstEvaluable(SLTypeCheckCtx* c) {
    SLTCConstEvalCtx evalCtx;
    int32_t          child;
    memset(&evalCtx, 0, sizeof(evalCtx));
    evalCtx.tc = c;
